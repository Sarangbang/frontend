...
import { TermsContent, PrivacyContent } from './Agreements';
...
export default function SignUpForm() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white py-12 px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4 pt-4">
            <div>
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setIsTermsModalOpen(true)} className="text-sm font-medium text-pink-500 hover:text-pink-600">
                  [내용보기]
                </button>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <input id="agreePrivacy" name="agreePrivacy" type="checkbox" checked={formData.agreePrivacy} onChange={handleChange}
                    className="h-4 w-4 text-pink-600 focus:ring-pink-500 border-gray-300 rounded" />
                  <label htmlFor="agreePrivacy" className="ml-3 block text-sm text-gray-900">
                    개인정보 처리방침 동의 (필수)
                  </label>
                </div>
                <button type="button" onClick={() => setIsPrivacyModalOpen(true)} className="text-sm font-medium text-pink-500 hover:text-pink-600">
                  [내용보기]
                </button>
              </div>
      ...
      <Modal isOpen={isTermsModalOpen} onClose={() => setIsTermsModalOpen(false)} title="이용약관">
        <TermsContent />
      </Modal>
      <Modal isOpen={isPrivacyModalOpen} onClose={() => setIsPrivacyModalOpen(false)} title="개인정보 처리방침">
        <PrivacyContent />
      </Modal>
    </div>
  );
